...
- **Problem**: Cache misses occur when headers or prefixes change subtly.
- **Solution**: Places `cache_control: { "type": "ephemeral" }` breakpoints on system prompts and tool schemas deterministically for Anthropic Claude and compatible endpoints, unlocking up to 90% input token cost reductions.
...
| `memoryCacheAnchorEnabled` | boolean | `true` | Enable/disable prompt cache breakpoint anchoring |
