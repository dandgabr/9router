    ...
    items: [
      ...
      },
      {
        label: (d) => {
          ...
          return `Prompt Cache Breakpoint Anchor: ${on ? "ON" : "OFF"} → toggle`;
        },
        action: async (d) => {
          await toggleMemorySetting("memoryCacheAnchorEnabled", d?.settings?.memoryCacheAnchorEnabled !== false, "Cache Breakpoint Anchor");
          return true;
        }
      },
      {
